...
class Common
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        $currency = Cache::remember('currency', 60*60*24*365, function () use ($general_setting) {
            $currency_id = $general_setting->currency ?? 1;
            return \App\Models\Currency::find($currency_id) ?? \App\Models\Currency::first();
        });
        ...
        // Fixed config with default values
        config([
            'staff_access' => $general_setting->staff_access ?? 'both',
            ...
            'company_name' => $general_setting->company_name ?? 'Your Company',
            'vat_registration_number' => $general_setting->vat_registration_number ?? '',
            'without_stock' => $general_setting->without_stock ?? 0,
