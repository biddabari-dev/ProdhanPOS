								...
								<tr>
									...
									<td>
										<input type="text" name="products[{{$loop->index}}][quantity]"
											   ...
											   class="form-control input-sm input_number total_return_qty"
									...
									<td>
										<input type="text" name="products[{{$loop->index}}][quantity_good]"
											   value="{{@format_quantity($sell_line->quantity_returned_good)}}"
											   class="form-control input-sm input_number return_qty_good"
											   data-rule-abs_digit="{{$check_decimal}}"
											   data-max-quantity="{{$sell_line->quantity}}"
											   placeholder="Good">
									</td>
									<td>
										<input type="text" name="products[{{$loop->index}}][quantity_damage]"
											   value="{{@format_quantity($sell_line->quantity_returned_damage)}}"
											   class="form-control input-sm input_number return_qty_damage"
											   data-rule-abs_digit="{{$check_decimal}}"
											   data-max-quantity="{{$sell_line->quantity}}"
											   placeholder="Damage">
									</td>
									<td>
										<input type="text" name="products[{{$loop->index}}][quantity_missing]"
											   value="{{@format_quantity($sell_line->quantity_returned_missing)}}"
											   class="form-control input-sm input_number return_qty_missing"
											   data-rule-abs_digit="{{$check_decimal}}"
											   data-max-quantity="{{$sell_line->quantity}}"
											   placeholder="Missing">
									</td>
...
@section('javascript')
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// Calculate total when any quantity field changes
			$('.return_qty_good, .return_qty_damage, .return_qty_missing').on('change keyup', function() {
				update_total_quantity($(this));
				update_sell_return_total();
			});
		});

		function update_total_quantity(element) {
			var row = element.closest('tr');
			var good = parseFloat(row.find('.return_qty_good').val()) || 0;
			var damage = parseFloat(row.find('.return_qty_damage').val()) || 0;
			var missing = parseFloat(row.find('.return_qty_missing').val()) || 0;

			var total = good + damage + missing;
			row.find('.total_return_qty').val(total.toFixed(4));

			// Validate max quantity
			var max_qty = parseFloat(row.find('.total_return_qty').data('max-value'));
			if (total > max_qty) {
				alert('Total return quantity cannot exceed ' + max_qty);
				element.val(0);
				update_total_quantity(element);
			}
		}
		...
		function update_sell_return_total() {
			...
			$('table#sell_return_table tbody tr').each(function() {
				var quantity = __read_number($(this).find('input.total_return_qty'));
